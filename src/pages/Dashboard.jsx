...
const Dashboard = () => {
      ...
      if (userAnalytics && Date.now() - userAnalytics.timestamp < 1000 * 60 * 30) {
        ...
        try {
          const response = await fetch(
            'https://resolute-land-440916-q3.el.r.appspot.com/analytics/getanalytics',
            {
              method: 'POST',
              headers: {
          ...
          localStorage.setItem(
            'userAnalytics',
            JSON.stringify({ ...dummyData, timestamp: Date.now() })
          );
  ...
  const CustomPolarAngleAxisTick = (props) => {
    ...
    return (
      <text x={x} y={y} textAnchor='middle' fill={`${props.stroke}`}>
        {lines.map((line, index) => (
          <tspan key={index} x={x} dy={index ? 12 : 0}>
            {line}
          </tspan>
        ))}
      </text>
    ...
    !isLoading && (
      <div className='flex min-h-screen'>
        ...
        <div className='min-h-screen w-full bg-gradient-to-br from-gray-900 to-blue-900 text-white p-8'>
          <Header
            name={user.result ? user.result.Name : 'user'}
            avatar={data.avatar}
            streak={data.streak}
          />
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6'>
            <MetricCard
              title='Focus Score'
              value={((userData.result && userData.result.overallScores.focusScore) || 0).toFixed(
                2
              )}
              icon='🎯'
              color='text-blue-400'
            ...
            <MetricCard
              title='Productivity'
              value={(
                (userData.result && userData.result.overallScores.productivityScore) ||
                0
              ).toFixed(2)}
              icon='📈'
            ...
            <MetricCard
              title='Wellbeing'
              value={(
                (userData.result && userData.result.overallScores.wellbeingScore) ||
                0
              ).toFixed(2)}
              icon='🌿'
            ...
            <MetricCard
              ...
              value={((userData.result && userData.result.overallScores.contentScore) || 0).toFixed(
                2
              )}
              icon='📚'
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6'>
            ...
            <ChartCard title='Content Consumption'>
              <ResponsiveContainer width='100%' height={250}>
                <BarChart
                  data={convertCategoryData(userData.result?.categoryData || {})}
                  layout='vertical'
                  margin={{ left: 50 }}
                >
                  <CartesianGrid strokeDasharray='3 3' stroke='#2a3a5a' />
                  ...
                  <YAxis
                    ...
                    width={30}
                    interval={0}
                    tick={{ fontSize: 15 }} // Decrease font size
                    tickFormatter={(value) =>
                      value.length > 10 ? `${value.slice(0, 5)}...` : value
                    }
                  />
                  <Tooltip
                    contentStyle={{ backgroundColor: '#1a2035', border: 'none' }}
              ...
              <ResponsiveContainer width='100%' height={300}>
                <RadarChart data={data.wellbeingData}>
                  ...
                  <PolarAngleAxis
                    dataKey='subject'
                    stroke='#8884d8'
                    tick={<CustomPolarAngleAxisTick stroke='#8884d8' />}
                  />
              ...
              <ResponsiveContainer width='100%' height={300}>
                <RadarChart data={data.productivityData}>
                  ...
                  <PolarAngleAxis
                    dataKey='subject'
                    stroke='#82ca9d'
                    tick={<CustomPolarAngleAxisTick stroke='#82ca9d' />}
                  />
              ...
              <ResponsiveContainer width='100%' height={300}>
                <RadarChart data={AnalyticsData.creativity}>
                  ...
                  <PolarAngleAxis
                    dataKey='subject'
                    stroke='#AF52DE'
                    tick={<CustomPolarAngleAxisTick stroke='#AF52DE' />}
                  />
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6'>
            <TopApps
              apps={getTopVisitedDomains(userData.result?.visitedDomains) || []}
              timeRange={timeRange}
            />
